...
HOST = "127.0.0.1"
# Configurable so the bridge and MCP server can move off 9876 together
# (set RESOLVE_BRIDGE_PORT on both sides).
PORT = int(os.environ.get("RESOLVE_BRIDGE_PORT", "9876"))
# Bump together with resolve_mcp_bridge.EXPECTED_BRIDGE_VERSION on every
# wire-protocol change so version drift is detected on connect.
BRIDGE_VERSION = "2.1.0"


# ---------------------------------------------------------------------------
# Logging — Resolve's script console is invisible to most users, so mirror
# every important message to a rotating log file as well as printing it.
# ---------------------------------------------------------------------------
def _log_path():
    env = os.environ.get("RESOLVE_BRIDGE_LOG")
    if env:
        return env
    ...
    try:
        ...
        return os.path.join(logs, "cursor_bridge.log")
    except Exception:
        import tempfile
        return os.path.join(tempfile.gettempdir(), "cursor_bridge.log")


_logger = logging.getLogger("cursor-bridge")
...
def log(msg, level=logging.INFO):
    """Log to the rotating file and also print to Resolve's console."""
    try:
...
def _find_ffmpeg():
    """Locate ffmpeg for reporting in /status (AI tools run MCP-side but this
    surfaces the situation early)."""
...
if resolve_obj:
    log("[CursorBridge] Connected to Resolve: %s %s" % (
        resolve_obj.GetProductName(), resolve_obj.GetVersionString()))
else:
    log("[CursorBridge] WARNING: Could not obtain Resolve object.", logging.WARNING)
...
def gather_status():
    ffmpeg = _find_ffmpeg()
    ffmpeg_info = {"found": ffmpeg is not None, "path": ffmpeg}
    r, err = _resolve()
    if err:
        return {
            "connected": False, "bridgeVersion": BRIDGE_VERSION,
            "port": PORT, "ffmpeg": ffmpeg_info, **err,
        }
    return {
        "connected": True,
...
def _shutdown_previous_instance():
    """Ask any previous bridge on this port to shut down, then wait briefly."""
    ...
    try:
        ...
        log("[CursorBridge] Sent shutdown to previous instance, waiting...")
        time.sleep(1.5)
        return True
    except Exception:
        return False
...
log("[CursorBridge] Starting HTTP server on http://%s:%d ..." % (HOST, PORT))
server = None
try:
    # ThreadingHTTPServer so one slow request cannot block every other call.
    # NOTE: the Resolve scripting API is not guaranteed thread-safe; write
    # handlers that mutate Resolve should stay short. Long AI/ffmpeg work is
    # dispatched to background jobs (see the JOBS registry) which is safe.
    server = ThreadingHTTPServer((HOST, PORT), BridgeHandler)
    server.daemon_threads = True
    log("[CursorBridge] Bridge is running (read + write).  %d GET routes, %d POST routes." % (
        len(GET_ROUTES), len(POST_ROUTES)))
    log("[CursorBridge] Version %s on port %d. To stop: close Resolve or re-run this script." % (
        BRIDGE_VERSION, PORT))
    server.serve_forever()
except OSError as e:
    if "Address already in use" in str(e) or "Only one usage" in str(e) or getattr(e, "errno", 0) == 10048:
        log("[CursorBridge] Port %d already in use — could not replace old bridge." % PORT,
            logging.ERROR)
        log("[CursorBridge] Another CursorBridge instance may be running. "
            "Restart DaVinci Resolve and try again, or set RESOLVE_BRIDGE_PORT.",
            logging.ERROR)
    else:
        log("[CursorBridge] ERROR: %s" % e, logging.ERROR)
except KeyboardInterrupt:
    log("[CursorBridge] Shutting down.")
