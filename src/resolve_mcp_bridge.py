...
# Port is configurable so the bridge and MCP server can be moved off 9876 in
# tandem (set RESOLVE_BRIDGE_PORT on both sides). See CursorBridge.py.
BRIDGE_PORT = int(os.environ.get("RESOLVE_BRIDGE_PORT", "9876"))
BRIDGE_URL = f"http://127.0.0.1:{BRIDGE_PORT}"

# Bump together with CursorBridge.BRIDGE_VERSION on every change to the wire
# protocol so a stale bridge or stale MCP server is detected on connect.
...
def _resolve_running() -> Optional[bool]:
    """Best-effort check for a running Resolve.exe process (Windows).

    Returns True/False when determinable, or None if the check itself failed
    (e.g. tasklist unavailable), so callers can fall back to a generic message.
    """
    try:
        out = subprocess.run(
...
def _conn_error() -> str:
    """Tailor the connection-failure message based on whether Resolve is up."""
    running = _resolve_running()
    if running is False:
        return (
            "DaVinci Resolve does not appear to be running. Launch Resolve, then "
            "start the bridge via Workspace > Scripts > CursorBridge."
        )
    if running is True:
        return (
            "DaVinci Resolve is running but the CursorBridge script isn't started. "
            "In Resolve: Workspace > Scripts > CursorBridge (the Console should show "
            "'Bridge is running')."
        )
    return CONN_ERROR
...
# ---------------------------------------------------------------------------
# By default the ~160 individual operations are consolidated into ~17 grouped
# tools (see the bottom of this file) to keep the per-session tool-schema
# context cost low. Set RESOLVE_MCP_LEGACY_TOOLS=1 to ALSO expose the original
# flat tools (useful during transition / debugging).
LEGACY_TOOLS = os.environ.get("RESOLVE_MCP_LEGACY_TOOLS", "0") == "1"


def flat_tool(fn):
    """Register a flat implementation function as an MCP tool only when
    LEGACY_TOOLS is enabled; otherwise leave it as a plain callable that the
    grouped dispatch tools call directly."""
    if LEGACY_TOOLS:
        return mcp.tool()(fn)
    return fn
...
def _get(endpoint: str, params: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
    ...
    if params:
        # urlencode handles spaces, & and # in clip names etc.
...
# ---------------------------------------------------------------------------
# Async job registry — long local-AI ops (transcription, voice isolation,
# background removal) run in a background thread so the MCP tool call returns
# immediately instead of tripping the client's tool-call timeout. Poll with
# get_job_status / list_jobs. The MCP server process persists between tool
# calls, so the worker thread keeps running after the tool has returned.
# ---------------------------------------------------------------------------
_JOBS: Dict[str, Dict[str, Any]] = {}
_JOBS_LOCK = threading.Lock()
_JOBS_MAX = 200  # cap retained finished jobs to avoid unbounded growth
...
def _run_async(kind: str, fn) -> Dict[str, Any]:
    """Run fn() in a daemon thread; return a job handle immediately.

    fn must return a dict (the eventual result). A returned dict containing an
    'error' key marks the job 'error'.
    """
    job_id = uuid.uuid4().hex[:12]
    with _JOBS_LOCK:
        _JOBS[job_id] = {
            "id": job_id, "kind": kind, "state": "queued", "progress": None,
            "result": None, "error": None, "started": None, "finished": None,
        }
        _prune_jobs()
    ...
    def _worker() -> None:
        ...
        try:
            ...
            with _JOBS_LOCK:
                ...
                _JOBS[job_id]["state"] = "error" if is_err else "done"
                if is_err:
                    _JOBS[job_id]["error"] = result.get("error")
                _JOBS[job_id]["finished"] = time.time()
        except Exception as e:  # noqa: BLE001
            logger.exception("Async job %s (%s) crashed", job_id, kind)
            with _JOBS_LOCK:
    ...
    return {
        ...
        "kind": kind,
        "state": "queued",
        "note": ("Long-running job started. Poll get_job_status(job_id) until "
...
def get_job_status(job_id: str) -> Dict[str, Any]:
    """Get the status/result of an async job started with async_job=True
    (transcription, voice isolation, background removal).
    Returns: state (queued|running|done|error), 'result' once done, 'error' if failed."""
    with _JOBS_LOCK:
...
def list_jobs() -> Dict[str, Any]:
    """List every async job this MCP server has tracked this session (newest state)."""
    with _JOBS_LOCK:
...
def get_resolve_status() -> Dict[str, Any]:
    ...
    Also reports the local ffmpeg path (needed by the ai_local tools) and warns
    if the bridge version does not match what this MCP server expects."""
    status = _get("/status")
    if not isinstance(status, dict):
        return {"error": "Unexpected status response", "raw": status}
    # Local-side info the bridge cannot know about.
    status["ffmpeg"] = {"found": FFMPEG_BIN is not None, "path": FFMPEG_BIN}
    status["mcpExpectedBridgeVersion"] = EXPECTED_BRIDGE_VERSION
    bridge_version = status.get("bridgeVersion")
    if bridge_version and bridge_version != EXPECTED_BRIDGE_VERSION:
        status["versionMismatch"] = (
            f"CursorBridge is v{bridge_version} but this MCP server expects "
            f"v{EXPECTED_BRIDGE_VERSION}. Copy the updated CursorBridge.py into "
            f"Resolve's Fusion\\Scripts\\Utility folder and re-run it (run install.ps1), "
            f"then restart Resolve's bridge. If the MCP server is the stale side, "
...
def voice_isolate(
    ...
    async_job=True (default) returns a job id immediately — poll get_job_status(job_id). Set False to block until done.
    Args:
    ...
    if async_job:
        return _run_async(
            "voice_isolate",
            lambda: _run_voice_isolation(file_path, model, stems, output_dir),
        )
...
def voice_isolate_timeline(
    ...
    async_job=True (default) returns a job id immediately — poll get_job_status(job_id).
    Args:
...
def remove_background(
    ...
    async_job=True returns a job id (poll get_job_status); default False (single images are usually quick).
    Args:
    ...
    if async_job:
        return _run_async(
            "remove_background",
            lambda: remove_background(file_path, model, output_path, alpha_matting, async_job=False),
        )
    try:
        from rembg import remove
        from PIL import Image
...
def remove_background_video(
    ...
    async_job=True (default) returns a job id immediately — poll get_job_status(job_id). This op can take minutes.
    Args:
    ...
    if async_job:
        return _run_async(
            ...
            lambda: remove_background_video(
                file_path, model, output_dir, output_format, alpha_matting, async_job=False),
        )
    try:
        from rembg import remove
        from PIL import Image
...
def remove_background_clip(
    ...
    Returns a job id (async) or the output directory and frame count (sync)."""
    ...
    return remove_background_video(
        file_path, model=model, output_format=output_format, async_job=async_job)
...
@flat_tool
def transcribe_timeline(model_size: str = "small", language: str = "",
                        async_job: bool = True) -> Dict[str, Any]:
    ...
    if async_job:
        return _run_async(
            "transcribe_timeline",
            lambda: _run_transcription(file_path, model_size, language or None),
        )
    return _run_transcription(file_path, model_size, language or None)


@flat_tool
def transcribe_file(file_path: str, model_size: str = "small", language: str = "",
                    async_job: bool = True) -> Dict[str, Any]:
...
# ═══════════════════════════════════════════════════════════════════════════
# GROUPED TOOLS  (consolidation — see LEGACY_TOOLS above)
# ═══════════════════════════════════════════════════════════════════════════
# Each grouped tool takes an `action` selector plus the union of parameters its
# actions need (all optional). It dispatches to the flat implementation
# functions defined above; _dispatch filters the provided params down to those
# the target function actually accepts (dropping None), so callers only pass
# what an action needs. This keeps ~160 operations behind ~17 tool schemas.


def _dispatch(fn, params: Dict[str, Any]) -> Dict[str, Any]:
    sig = inspect.signature(fn)
    accepts = {k: v for k, v in params.items()
               if k in sig.parameters and v is not None}
    return fn(**accepts)


# Wrappers for functions whose own first parameter is literally named `action`
# (which would collide with the group selector). They expose a renamed param.
def _wrap_project_navigate(folder_action=None, folder_name=None):
    return navigate_project_folder(action=folder_action, folder_name=folder_name)


def _wrap_layout_preset(preset_action=None, preset_name=None, preset_file_path=None):
    return layout_preset(action=preset_action, preset_name=preset_name,
                         preset_file_path=preset_file_path)


def _wrap_burnin_preset(preset_action=None, preset_name=None, preset_path=None):
    return burnin_preset(action=preset_action, preset_name=preset_name,
                         preset_path=preset_path)


def _wrap_render_preset(preset_action=None, preset_name=None, preset_path=None):
    return render_preset(action=preset_action, preset_name=preset_name,
                         preset_path=preset_path)


_GROUP_TABLES: Dict[str, Dict[str, Any]] = {
    "resolve": {
        "status": get_resolve_status, "current_page": get_current_page,
        "open_page": open_page, "layout_preset": _wrap_layout_preset,
        "burnin_preset": _wrap_burnin_preset, "database_list": get_database_list,
        "set_database": set_database, "keyframe_mode_get": get_keyframe_mode,
        "keyframe_mode_set": set_keyframe_mode,
    },
    "project": {
        "list": get_project_list, "load": load_project, "create": create_project,
        "delete": delete_project, "save": save_project, "info": get_project_info,
        "export": export_project, "import": import_project, "archive": archive_project,
        "set_setting": set_project_setting, "navigate_folder": _wrap_project_navigate,
    },
    "media_pool": {
        "get": get_media_pool, "structure": get_media_pool_structure,
        "navigate": navigate_media_pool, "create_folder": create_media_pool_folder,
        "import": import_media, "import_from_storage": import_media_from_storage,
        "storage_browse": get_media_storage, "storage_reveal": reveal_in_storage,
        "move": move_media_pool_clips, "delete": delete_media_pool_clips,
        "relink": relink_media_pool_clips, "unlink": unlink_media_pool_clips,
        "link_proxy": link_proxy_media, "unlink_proxy": unlink_proxy_media,
        "replace": replace_clip, "get_clip_property": get_clip_info,
        "set_clip_property": set_pool_clip_property,
    },
    "timeline": {
        "create": create_timeline, "duplicate": duplicate_timeline,
        "rename": rename_timeline, "switch": switch_timeline, "info": get_timeline_info,
        "clips": get_timeline_clips, "current_item": get_current_video_item,
        "thumbnail": get_clip_thumbnail, "export": export_timeline,
        "import_from_file": import_timeline_from_file, "set_setting": set_timeline_setting,
        "mark_in_out": set_timeline_mark_in_out, "clear_mark_in_out": clear_timeline_mark_in_out,
        "playhead": set_playhead, "detect_scene_cuts": detect_scene_cuts,
    },
    "timeline_edit": {
        "append": append_to_timeline, "insert": insert_to_timeline,
        "insert_title": insert_title, "insert_generator": insert_generator,
        "insert_fusion_composition": insert_fusion_composition,
        "insert_audio_at_playhead": insert_audio_at_playhead,
        "delete_clips": delete_timeline_clips, "replace_clip": replace_clip,
        "compound_clip": create_compound_clip, "fusion_clip": create_fusion_clip,
        "link_clips": link_timeline_clips,
    },
    "tracks": {
        "add": add_track, "delete": delete_track, "name": set_track_name,
        "lock": set_track_lock, "enable": set_track_enable,
    },
    "clip": {
        "properties_get": get_clip_properties, "properties_set": set_clip_properties,
        "enabled": set_clip_enabled, "color": set_clip_color,
        "flags_get": get_clip_flags, "flag_add": add_clip_flag,
        "flags_clear": clear_clip_flags, "cache": set_clip_cache,
        "stabilize": stabilize_clip, "smart_reframe": smart_reframe_clip,
        "sidecar": update_sidecar, "linked_items": get_linked_items,
    },
    "markers": {
        "timeline_add": add_marker, "timeline_get": get_timeline_markers,
        "timeline_delete": delete_markers, "clip_add": add_clip_marker,
        "clip_get": get_clip_markers, "clip_delete": delete_clip_markers,
    },
    "takes": {
        "get": get_takes, "add": add_take, "select": select_take,
        "delete": delete_take, "finalize": finalize_take,
    },
    "color": {
        "node_graph": get_node_graph, "set_lut": set_lut, "get_lut": get_lut,
        "set_node_enabled": set_node_enabled, "apply_drx": apply_grade_from_drx,
        "reset_grades": reset_all_grades, "apply_arri_cdl": apply_arri_cdl_lut,
        "set_cdl": set_cdl, "export_lut": export_lut, "copy_grades": copy_grades,
        "reset_node_colors": reset_node_colors, "refresh_luts": refresh_lut_list,
        "versions_get": get_color_versions, "version_add": add_color_version,
        "version_load": load_color_version, "version_delete": delete_color_version,
        "version_rename": rename_color_version,
    },
    "color_groups": {
        "list": get_color_groups, "add": add_color_group, "delete": delete_color_group,
        "assign": assign_to_color_group, "remove": remove_from_color_group,
    },
    "gallery": {
        "albums": get_gallery_albums, "album_create": create_gallery_album,
        "album_set": set_current_album, "stills_get": get_album_stills,
        "grab": grab_still, "grab_all": grab_all_stills, "stills_export": export_stills,
        "stills_import": import_stills, "stills_delete": delete_stills,
        "still_label": set_still_label,
    },
    "fusion": {
        "comps": get_fusion_comps, "add": add_fusion_comp_to_clip,
        "import": import_fusion_comp_to_clip, "export": export_fusion_comp_from_clip,
        "delete": delete_fusion_comp_on_clip, "load": load_fusion_comp_on_clip,
        "rename": rename_fusion_comp_on_clip, "magic_mask": create_magic_mask,
        "magic_mask_regenerate": regenerate_magic_mask,
    },
    "fairlight": {
        "presets": get_fairlight_presets, "apply_preset": apply_fairlight_preset,
        "voice_isolation_get": get_voice_isolation_state,
        "voice_isolation_set": set_voice_isolation_state,
    },
    "render": {
        "settings_get": get_render_settings, "settings_set": set_render_settings,
        "set_format": set_render_format, "formats": get_render_formats,
        "resolutions": get_render_resolutions, "add_job": add_render_job,
        "delete_job": delete_render_job, "job_status": get_render_job_status,
        "start": start_rendering, "stop": stop_rendering, "mode": set_render_mode,
        "quick_export_presets": get_quick_export_presets, "quick_export": quick_export,
        "preset": _wrap_render_preset,
    },
    "ai_local": {
        "transcribe_file": transcribe_file, "transcribe_timeline": transcribe_timeline,
        "create_subtitles": create_subtitles_from_audio, "voice_isolate": voice_isolate,
        "voice_isolate_timeline": voice_isolate_timeline,
        "remove_background": remove_background,
        "remove_background_video": remove_background_video,
        "remove_background_clip": remove_background_clip, "auto_sync_audio": auto_sync_audio,
        "job_status": get_job_status, "list_jobs": list_jobs,
    },
    "metadata": {
        "get": get_clip_metadata, "set": set_clip_metadata, "export": export_metadata,
        "update_sidecar": update_sidecar, "export_frame": export_current_frame,
    },
}


def _run_group(group: str, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
    table = _GROUP_TABLES.get(group, {})
    fn = table.get(action)
    if fn is None:
        return {"error": f"Unknown action '{action}' for '{group}'. "
                         f"Valid actions: {sorted(table)}"}
    return _dispatch(fn, params)


def _params_minus_action(local_vars: Dict[str, Any]) -> Dict[str, Any]:
    d = dict(local_vars)
    d.pop("action", None)
    return d
...
@mcp.tool()
def resolve(action: str, page: Optional[str] = None, preset_action: Optional[str] = None,
            preset_name: Optional[str] = None, preset_file_path: Optional[str] = None,
            preset_path: Optional[str] = None, db_type: Optional[str] = None,
            db_name: Optional[str] = None, ip_address: Optional[str] = None,
            mode: Optional[str] = None) -> Dict[str, Any]:
    """App-level Resolve control. action ∈ {status, current_page, open_page(page),
    layout_preset(preset_action,preset_name,preset_file_path),
    burnin_preset(preset_action,preset_name,preset_path), database_list,
    set_database(db_type,db_name,ip_address), keyframe_mode_get, keyframe_mode_set(mode)}.
    Call action='status' first to confirm the bridge is up."""
    return _run_group("resolve", action, _params_minus_action(locals()))


@mcp.tool()
def project(action: str, project_name: Optional[str] = None, file_path: Optional[str] = None,
            with_stills_and_luts: Optional[bool] = None, archive_src_media: Optional[bool] = None,
            archive_render_cache: Optional[bool] = None, archive_proxy_media: Optional[bool] = None,
            key: Optional[str] = None, value: Optional[Any] = None,
            folder_action: Optional[str] = None, folder_name: Optional[str] = None) -> Dict[str, Any]:
    """Project management. action ∈ {list, load(project_name), create(project_name),
    delete(project_name), save, info, export(project_name,file_path,with_stills_and_luts),
    import(file_path,project_name), archive(project_name,file_path,archive_*),
    set_setting(key,value), navigate_folder(folder_action,folder_name)}."""
    return _run_group("project", action, _params_minus_action(locals()))


@mcp.tool()
def media_pool(action: str, file_paths: Optional[List[str]] = None, path: Optional[str] = None,
               name: Optional[str] = None, parent_path: Optional[str] = None,
               include_clips: Optional[bool] = None, max_depth: Optional[int] = None,
               folder_path: Optional[str] = None, clip_names: Optional[List[str]] = None,
               target_folder: Optional[str] = None, clip_name: Optional[str] = None,
               proxy_media_file_path: Optional[str] = None, property_name: Optional[str] = None,
               property_value: Optional[Any] = None) -> Dict[str, Any]:
    """Media pool + media storage. action ∈ {get, structure(include_clips,max_depth),
    navigate(path), create_folder(name,parent_path), import(file_paths),
    import_from_storage(file_paths), storage_browse(folder_path), storage_reveal(path),
    move(clip_names,target_folder), delete(clip_names), relink(clip_names,folder_path),
    unlink(clip_names), link_proxy(clip_name,proxy_media_file_path), unlink_proxy(clip_name),
    replace(clip_name,file_path), get_clip_property(clip_name),
    set_clip_property(clip_name,property_name,property_value)}."""
    return _run_group("media_pool", action, _params_minus_action(locals()))


@mcp.tool()
def timeline(action: str, name: Optional[str] = None, index: Optional[int] = None,
             track_type: Optional[str] = None, track_index: Optional[int] = None,
             file_name: Optional[str] = None, export_type: Optional[str] = None,
             export_subtype: Optional[str] = None, file_path: Optional[str] = None,
             import_options: Optional[Dict[str, Any]] = None, key: Optional[str] = None,
             value: Optional[Any] = None, mark_in: Optional[int] = None,
             mark_out: Optional[int] = None, type: Optional[str] = None,
             timecode: Optional[str] = None) -> Dict[str, Any]:
    """Timeline management (non-editing). action ∈ {create(name), duplicate(name),
    rename(name), switch(index), info, clips(track_type,track_index), current_item,
    thumbnail, export(file_name,export_type,export_subtype),
    import_from_file(file_path,import_options), set_setting(key,value),
    mark_in_out(mark_in,mark_out,type), clear_mark_in_out(type), playhead(timecode),
    detect_scene_cuts}."""
    return _run_group("timeline", action, _params_minus_action(locals()))


@mcp.tool()
def timeline_edit(action: str, clip_name: Optional[str] = None, track_index: Optional[int] = None,
                  record_frame: Optional[int] = None, start_frame: Optional[int] = None,
                  end_frame: Optional[int] = None, title_name: Optional[str] = None,
                  fusion_title: Optional[bool] = None, generator_name: Optional[str] = None,
                  fusion_generator: Optional[bool] = None, media_path: Optional[str] = None,
                  start_offset_in_samples: Optional[int] = None,
                  duration_in_samples: Optional[int] = None, clips: Optional[List[Any]] = None,
                  ripple: Optional[bool] = None, file_path: Optional[str] = None,
                  name: Optional[str] = None, start_timecode: Optional[str] = None,
                  linked: Optional[bool] = None) -> Dict[str, Any]:
    """Timeline editing. action ∈ {append(clip_name),
    insert(clip_name,track_index,record_frame,start_frame,end_frame),
    insert_title(title_name,fusion_title), insert_generator(generator_name,fusion_generator),
    insert_fusion_composition, insert_audio_at_playhead(media_path,start_offset_in_samples,
    duration_in_samples), delete_clips(clips,ripple), replace_clip(clip_name,file_path),
    compound_clip(clips,name,start_timecode), fusion_clip(clips), link_clips(clips,linked)}.
    'clips' is a list of {trackType,trackIndex,clipIndex} dicts."""
    return _run_group("timeline_edit", action, _params_minus_action(locals()))


@mcp.tool()
def tracks(action: str, track_type: Optional[str] = None, track_index: Optional[int] = None,
           sub_track_type: Optional[str] = None, name: Optional[str] = None,
           locked: Optional[bool] = None, enabled: Optional[bool] = None) -> Dict[str, Any]:
    """Track management. action ∈ {add(track_type,sub_track_type), delete(track_type,track_index),
    name(track_type,track_index,name), lock(track_type,track_index,locked),
    enable(track_type,track_index,enabled)}."""
    return _run_group("tracks", action, _params_minus_action(locals()))


@mcp.tool()
def clip(action: str, track_type: Optional[str] = None, track_index: Optional[int] = None,
         clip_index: Optional[int] = None, properties: Optional[Dict[str, Any]] = None,
         enabled: Optional[bool] = None, color: Optional[str] = None,
         cache_type: Optional[str] = None, cache_value: Optional[Any] = None) -> Dict[str, Any]:
    """Timeline-clip properties (track_type,track_index,clip_index identify the clip).
    action ∈ {properties_get, properties_set(properties), enabled(enabled), color(color),
    flags_get, flag_add(color), flags_clear(color), cache(cache_type,cache_value),
    stabilize, smart_reframe, sidecar, linked_items}."""
    return _run_group("clip", action, _params_minus_action(locals()))


@mcp.tool()
def markers(action: str, track_type: Optional[str] = None, track_index: Optional[int] = None,
            clip_index: Optional[int] = None, frameId: Optional[int] = None,
            color: Optional[str] = None, name: Optional[str] = None, note: Optional[str] = None,
            duration: Optional[int] = None, customData: Optional[str] = None) -> Dict[str, Any]:
    """Markers. action ∈ {timeline_add(frameId,color,name,note,duration,customData),
    timeline_get, timeline_delete(frameId|color), clip_add(track_*,clip_index,frameId,...),
    clip_get(track_*,clip_index), clip_delete(track_*,clip_index,frameId|color|customData)}."""
    return _run_group("markers", action, _params_minus_action(locals()))


@mcp.tool()
def takes(action: str, track_type: Optional[str] = None, track_index: Optional[int] = None,
          clip_index: Optional[int] = None, media_pool_clip_name: Optional[str] = None,
          start_frame: Optional[int] = None, end_frame: Optional[int] = None,
          take_index: Optional[int] = None) -> Dict[str, Any]:
    """Take selector on a clip. action ∈ {get, add(media_pool_clip_name,start_frame,end_frame),
    select(take_index), delete(take_index), finalize} (all take track_type,track_index,clip_index)."""
    return _run_group("takes", action, _params_minus_action(locals()))


@mcp.tool()
def color(action: str, track_type: Optional[str] = None, track_index: Optional[int] = None,
          clip_index: Optional[int] = None, scope: Optional[str] = None,
          layer_index: Optional[int] = None, node_index: Optional[int] = None,
          lut_path: Optional[str] = None, enabled: Optional[bool] = None,
          drx_path: Optional[str] = None, grade_mode: Optional[int] = None,
          cdl: Optional[Dict[str, str]] = None, path: Optional[str] = None,
          export_type: Optional[str] = None, source: Optional[Dict[str, Any]] = None,
          targets: Optional[List[Any]] = None, version_name: Optional[str] = None,
          version_type: Optional[str] = None, old_name: Optional[str] = None,
          new_name: Optional[str] = None) -> Dict[str, Any]:
    """Color grading (no LUT needed for CDL). action ∈ {node_graph, set_lut(node_index,lut_path),
    get_lut(node_index), set_node_enabled(node_index,enabled), apply_drx(drx_path,grade_mode),
    reset_grades, apply_arri_cdl, set_cdl(cdl), export_lut(path,export_type),
    copy_grades(source,targets), reset_node_colors, refresh_luts, versions_get,
    version_add/load/delete(version_name,version_type), version_rename(old_name,new_name,version_type)}.
    cdl = {NodeIndex,Slope,Offset,Power,Saturation}. Most actions take track_type,track_index,clip_index."""
    return _run_group("color", action, _params_minus_action(locals()))


@mcp.tool()
def color_groups(action: str, group_name: Optional[str] = None, track_type: Optional[str] = None,
                 track_index: Optional[int] = None, clip_index: Optional[int] = None) -> Dict[str, Any]:
    """Color groups. action ∈ {list, add(group_name), delete(group_name),
    assign(track_*,clip_index,group_name), remove(track_*,clip_index)}."""
    return _run_group("color_groups", action, _params_minus_action(locals()))


@mcp.tool()
def gallery(action: str, album_type: Optional[str] = None, name: Optional[str] = None,
            album_index: Optional[int] = None, still_frame_source: Optional[int] = None,
            folder_path: Optional[str] = None, file_prefix: Optional[str] = None,
            format: Optional[str] = None, still_indices: Optional[List[int]] = None,
            file_paths: Optional[List[str]] = None, still_index: Optional[int] = None,
            label: Optional[str] = None) -> Dict[str, Any]:
    """Gallery / stills. action ∈ {albums, album_create(album_type,name),
    album_set(album_index,album_type), stills_get(album_index,album_type), grab,
    grab_all(still_frame_source), stills_export(folder_path,file_prefix,format,...),
    stills_import(file_paths), stills_delete(still_indices,...), still_label(still_index,label)}."""
    return _run_group("gallery", action, _params_minus_action(locals()))


@mcp.tool()
def fusion(action: str, track_type: Optional[str] = None, track_index: Optional[int] = None,
           clip_index: Optional[int] = None, path: Optional[str] = None,
           comp_index: Optional[int] = None, comp_name: Optional[str] = None,
           old_name: Optional[str] = None, new_name: Optional[str] = None,
           mode: Optional[str] = None) -> Dict[str, Any]:
    """Fusion comps on a clip. action ∈ {comps, add, import(path), export(path,comp_index),
    delete(comp_name), load(comp_name), rename(old_name,new_name), magic_mask(mode),
    magic_mask_regenerate} (all take track_type,track_index,clip_index)."""
    return _run_group("fusion", action, _params_minus_action(locals()))


@mcp.tool()
def fairlight(action: str, preset_name: Optional[str] = None, scope: Optional[str] = None,
              track_type: Optional[str] = None, track_index: Optional[int] = None,
              clip_index: Optional[int] = None, is_enabled: Optional[bool] = None,
              amount: Optional[float] = None) -> Dict[str, Any]:
    """Fairlight audio. action ∈ {presets, apply_preset(preset_name),
    voice_isolation_get(scope,track_*,clip_index),
    voice_isolation_set(scope,track_*,clip_index,is_enabled,amount)}."""
    return _run_group("fairlight", action, _params_minus_action(locals()))


@mcp.tool()
def render(action: str, settings: Optional[Dict[str, Any]] = None, format: Optional[str] = None,
           codec: Optional[str] = None, job_id: Optional[str] = None, all: Optional[bool] = None,
           job_ids: Optional[List[str]] = None, render_mode: Optional[str] = None,
           preset_name: Optional[str] = None, params: Optional[Dict[str, Any]] = None,
           preset_action: Optional[str] = None, preset_path: Optional[str] = None) -> Dict[str, Any]:
    """Rendering. action ∈ {settings_get, settings_set(settings), set_format(format,codec),
    formats(format), resolutions(format,codec), add_job, delete_job(job_id,all),
    job_status(job_id), start(job_ids), stop, mode(render_mode), quick_export_presets,
    quick_export(preset_name,params), preset(preset_action,preset_name,preset_path)}."""
    return _run_group("render", action, _params_minus_action(locals()))


@mcp.tool()
def ai_local(action: str, file_path: Optional[str] = None, model_size: Optional[str] = None,
             language: Optional[str] = None, async_job: Optional[bool] = None,
             model: Optional[str] = None, stems: Optional[str] = None,
             output_dir: Optional[str] = None, output_path: Optional[str] = None,
             alpha_matting: Optional[bool] = None, output_format: Optional[str] = None,
             track_type: Optional[str] = None, track_index: Optional[int] = None,
             clip_index: Optional[int] = None, clip_names: Optional[List[str]] = None,
             settings: Optional[Dict[str, Any]] = None, job_id: Optional[str] = None) -> Dict[str, Any]:
    """Local AI (CPU, no Studio needed) + async jobs. action ∈ {transcribe_file(file_path,
    model_size,language,async_job), transcribe_timeline(model_size,language,async_job),
    create_subtitles, voice_isolate(file_path,model,stems,output_dir,async_job),
    voice_isolate_timeline(...), remove_background(file_path,model,output_path,alpha_matting,async_job),
    remove_background_video(file_path,...), remove_background_clip(track_*,clip_index,model,...),
    auto_sync_audio(clip_names,settings), job_status(job_id), list_jobs}.
    Long ops default async_job=True → returns a job id; poll with action='job_status'."""
    return _run_group("ai_local", action, _params_minus_action(locals()))


@mcp.tool()
def metadata(action: str, clip_name: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None,
             file_name: Optional[str] = None, clip_names: Optional[List[str]] = None,
             track_type: Optional[str] = None, track_index: Optional[int] = None,
             clip_index: Optional[int] = None, file_path: Optional[str] = None) -> Dict[str, Any]:
    """Metadata + frame export. action ∈ {get(clip_name), set(clip_name,metadata),
    export(file_name,clip_names), update_sidecar(track_*,clip_index), export_frame(file_path)}."""
    return _run_group("metadata", action, _params_minus_action(locals()))
